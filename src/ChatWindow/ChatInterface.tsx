...
// // // // // //     const response = await fetch(`http://172.206.198.176/api/chat_history/?user_id=${user_id}`);
// // // // // //     if (!response.ok) {
...
// // // // // //     const response = await fetch(`http://172.206.198.176/api/chatgpt/`, {
// // // // // //       method: 'POST',
// // // // // //       body: formData,
// // // // // //     });
...
So, what would you like to predict?`;
  ...
  // Use the logged-in user id if available; otherwise, use the URL parameter.
  ...
  // When the generate button is visible, we disable chat input.
  const [disableChatInput, setDisableChatInput] = useState(false);
  ...
  // Helper: create a new chat object (without updating state)
  ...
  // ---------------------------------------------------------------------------
  // Initialize chat history: If URL has chat id, select that chat; if not, create new chat
  // while still showing the user's chat history.
  // ---------------------------------------------------------------------------
  const fetchAndInitializeChats = async () => {
    try {
      if (!userId) {
        // If no userId, create a new chat.
      ...
      if (fetchedChats.length > 0) {
        if (paramChatId) {
          // If URL has a chat id, select that chat.
          const selectedChat = fetchedChats.find((chat) => chat.id === paramChatId);
          if (selectedChat) {
            setChats(fetchedChats);
            setCurrentChat(selectedChat);
          } else {
            // Chat id not found: create a new chat and prepend to history.
            const newChat = createNewChatObject();
            setCurrentChat(newChat);
            setChats([newChat, ...fetchedChats]);
          }
        } else {
          // No chat id in URL: create a new chat but show history.
          const newChat = createNewChatObject();
          setCurrentChat(newChat);
          setChats([newChat, ...fetchedChats]);
        }
      } else {
        // No chats fetched: create a new chat.
        const newChat = createNewChatObject();
        setChats([newChat]);
        setCurrentChat(newChat);
      }
  ...
  // ---------------------------------------------------------------------------
  // When a user clicks on a chat in history, update currentChat and navigate.
  // ---------------------------------------------------------------------------
...
No date column was detected, so we'll proceed with a non-time-based approach.
  ...
  //     // If this is the first message in a new chat (i.e. URL did not have a chat id),
  //     // update the URL now.
  //     if (!paramChatId) {
  //       navigate(`/chat/${userId}/${currentChat.id}`);
  //     }
  //     // When the backend requests the generate notebook button to be shown,
  //     // disable the chat input.
  //     if (showGenerateButton) {
  ...
  const handleSendMessage = async () => {
    ...
    // Determine if this is a new chat. (No chat_id in the URL)
    ...
    try {
      ...
      // Use the new chat id from the response if provided.
      const responseText =
      ...
      // Update the chat with the new id and new messages.
      setChats((prevChats) =>
        prevChats.map((chat) =>
      ...
      // Use the new chat id in the URL so that paramChatId becomes defined.
      if (!paramChatId) {
  ...
  // ---------------------------------------------------------------------------
  const handleNewChat = () => {
    // When clicking New Chat, navigate to /chat so that URL has no chat id.
    navigate('/chat');
    // Do NOT clear the existing chat history in the sidebar.
    const newChat: Chat = {
      id: uuidv4(),
      title: 'New Prediction',
      timestamp: new Date().toLocaleString(),
    ...
    // Prepend the new chat to the existing chats.
    setChats((prev) => [newChat, ...prev]);
    setCurrentChat(newChat);
    setIsGeneratingNotebook(false);
    setNotebookGenerated(false);
    setGeneratedNotebookData(null);
    setDisableChatInput(false);
    // URL remains /chat until the first message is sent.
  };

  // ---------------------------------------------------------------------------
  // Generate Notebook (unchanged)
  // ---------------------------------------------------------------------------
  const handleGenerateNotebook = async () => {
    if (!currentChat || !userId) {
      console.error('User ID or Current Chat not available.');
...
const handleOpenNotebook = async () => {
  try {
    ...
    const response = await fetch(
      `http://172.206.198.176/api/predictive-settings/${userId}/${notebookChatId}/`
    );
    if (!response.ok) {
            ...
            <ScrollArea className="h-[calc(100vh-5rem)] px-2">
              <div className="space-y-2 p-2">
                {chats.map((chat) => (
                  ...
                  >
                    <Card className={`${currentChat?.id === chat.id ? 'bg-accent' : ''}`}>
                      <CardContent className="p-3 flex justify-between items-center">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{chat.title}</p>
                          <p className="text-xs text-muted-foreground">{chat.timestamp}</p>
