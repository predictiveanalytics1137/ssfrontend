...
const Homepage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Modern Sidebar */}
      <div className="fixed left-0 top-0 h-full w-16 bg-[#5B3557] flex flex-col items-center py-6 space-y-8">
        <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center">
          <span className="text-[#5B3557] font-bold">P</span>
        </div>
        <div className="space-y-6">
          <button className="w-10 h-10 rounded-lg bg-[#4A2E47] flex items-center justify-center">
            <Activity className="w-5 h-5 text-white" />
          </button>
          <button className="w-10 h-10 rounded-lg hover:bg-[#4A2E47]/30 flex items-center justify-center">
            <Timer className="w-5 h-5 text-white" />
          </button>
          <button className="w-10 h-10 rounded-lg hover:bg-[#4A2E47]/30 flex items-center justify-center">
            <Zap className="w-5 h-5 text-white" />
      ...
      <div className="ml-16 p-8">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative">
              <input
                type="text"
                placeholder="Search flows..."
                className="pl-10 pr-4 py-2 w-64 bg-white border-0 rounded-xl shadow-sm text-sm focus:ring-2 focus:ring-[#5B3557]"
              />
              <Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" />
            </div>
            ...
            <button 
              onClick={() => setShowNewFlow(!showNewFlow)}
              className="bg-[#5B3557] text-white px-4 py-2 rounded-xl shadow-sm hover:bg-[#4A2E47] flex items-center space-x-2"
            >
              <Plus className="w-4 h-4" />
              <span className="text-sm font-medium">New Flow</span>
            </button>
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-4">
            <button 
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-white shadow text-[#5B3557]' : 'text-slate-600'}`}
            >
              <LayoutGrid className="w-5 h-5" />
            </button>
            <button 
              onClick={() => setViewMode('list')}
              className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-white shadow text-[#5B3557]' : 'text-slate-600'}`}
            >
              <List className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex items-center space-x-3">
            <button className="px-4 py-1.5 rounded-full text-sm bg-white shadow-sm text-[#5B3557] border border-[#4A2E47]">
              All Flows
            </button>
            <button className="px-4 py-1.5 rounded-full text-sm hover:bg-white hover:shadow-sm text-slate-600">
              Production
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-[#5B3557] rounded-full"
                        style={{ width: `${flow.progress}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex items-center justify-between text-sm">
                    <span className="text-slate-500">Last run {flow.lastRun}</span>
                    <button 
                      className="text-[#5B3557] hover:text-[#4A2E47] flex items-center"
                      onClick={(e) => {
                        e.stopPropagation();
                        handleNotebookClick(flow);
                      }}
...
//   file_url: string;
//   created_at: string;
//   // Optional status attached later from the second fetch
//   status?: string;
// }
...
//   // Fetch dynamic user id from localStorage and then fetch notebooks
//   useEffect(() => {
...
//       setLoading(true);
//       try {
//         const url = `http://98.70.25.52/api/notebooks/?user_id=${uid}`;
//         const resp = await fetch(url);
//         if (!resp.ok) {
//           throw new Error(`Failed to fetch notebooks: ${resp.statusText}`);
//         }
//         const data = await resp.json();
//         // If the response doesn't contain notebooks, set an empty array
//         if (!data.notebooks) {
//           setNotebooks([]);
//         } else {
//           const rawNotebooks: Notebook[] = data.notebooks;

//           // For each notebook, fetch the status from the second endpoint
//           const notebooksWithStatus: Notebook[] = [];
//           for (const nb of rawNotebooks) {
//             // Use chat_id from notebook if it exists, otherwise fallback to notebook id
//             const chatId = nb.chat_id ? nb.chat_id.toString() : nb.id.toString();
//             try {
//               const metaUrl = `http://98.70.25.52/api/get_prediction_metadata/?user_id=${uid}&chat_id=${chatId}`;
//               const metaResp = await fetch(metaUrl);
//               if (metaResp.ok) {
//                 const metaData = await metaResp.json();
//                 const item = metaData.metadata?.[0];
//                 if (item?.status) {
//                   nb.status = item.status; // e.g., "step1", "step2", "success", etc.
//                 } else {
//                   nb.status = 'unknown';
//                 }
//               } else {
...
//               }
//             } catch (err) {
//               // If the second fetch fails, set status to 'unknown'
//               nb.status = 'unknown';
//             }
//             notebooksWithStatus.push(nb);
//           }
...
//   // Transform notebooks into flows for the UI
//   const flows = notebooks.map((nb) => {
//     let progress = 0;
//     if (nb.status === 'success') {
//       progress = 100;
...
//       const stepNum = parseInt(nb.status.replace('step', ''), 10);
//       if (!isNaN(stepNum)) {
//         progress = stepNum * 25; // e.g., step1=25, step2=50, step3=75
//       }
//     }
...
//       status: nb.status || 'pending',
//       progress,
//       lastRun: nb.created_at, // or another date/time field
//       type: nb.time_column ? 'Time-Based' : 'Non-time',
//       chat_id: nb.chat_id ? nb.chat_id : nb.id, // use dynamic chat_id or fallback to id
//     };
//   });
...
//       return;
//     }
//     // Navigate to /notebook/:userId/:chat_id with additional state if needed
//     navigate(`/notebook/${userId}/${flow.chat_id}`, {
//       state: {
//         isTrained: false, // Initially not trained
//       },
//     });
...
//   // Show a loading or error message when appropriate
//   if (loading) {
//     return <div className="p-8">Loading notebooks...</div>;
//   }
//   if (error) {
...
//         {viewMode === 'grid' && (
//           flows.length === 0 ? (
//             <div className="text-center text-slate-600">No notebooks found. Please create a new flow.</div>
//           ) : (
//             <div className="grid grid-cols-3 gap-6">
//               {flows.map((flow) => (
//                 <div 
...
//                           onClick={(e) => {
//                             e.stopPropagation();
//                             // Optionally, add a context menu here.
//                           }}
//                         >
//                           <MoreVertical className="w-4 h-4" />
//                         </button>
