...
const ModelEvaluation: React.FC<ModelEvaluationProps> = ({ modelData }) => {
  ...
  // Extract actual and predicted values with safety checks
  const actual = predictions?.actual && Array.isArray(predictions.actual) ? predictions.actual : [];
  const predicted = predictions?.predicted && Array.isArray(predictions.predicted) ? predictions.predicted : [];
  ...
  // Memoize chart data for performance
  const chartData = useMemo(() => {
    if (!actual.length || !predicted.length || actual.length !== predicted.length) return [];
    return actual.map((value: number, index: number) => ({
      index: index + 1,
      actual: value,
      predicted: predicted[index],
  ...
  // Calculate totals and bias with safety checks
  const totalActual = useMemo(() => {
    if (!actual.length) return "0.0";
    const sum = actual.reduce((acc, val) => acc + (typeof val === "number" ? val : 0), 0);
    return sum.toFixed(1);
  }, [actual]);

  const totalPredicted = useMemo(() => {
    if (!predicted.length) return "0.0";
    const sum = predicted.reduce((acc, val) => acc + (typeof val === "number" ? val : 0), 0);
    return sum.toFixed(1);
  }, [predicted]);

  const biasValue = useMemo(() => {
    if (!actual.length || totalActual === "0.0") return 0;
    const actualNum = Number(totalActual);
    const predictedNum = Number(totalPredicted);
    return ((predictedNum - actualNum) / actualNum) * 100;
  }, [totalActual, totalPredicted, actual]);

  // Determine which metrics to use dynamically
  const metricsToUse = model_metrics?.validation || model_metrics?.testing || null;

  // Extract metrics with precise formatting and fallback
  const mae = metricsToUse?.mae != null && !isNaN(metricsToUse.mae) ? Number(metricsToUse.mae).toFixed(1) : "N/A";
  const rmse = metricsToUse?.rmse != null && !isNaN(metricsToUse.rmse) ? Number(metricsToUse.rmse).toFixed(1) : "N/A";
  const r2Score = metricsToUse?.r2_score != null && !isNaN(metricsToUse.r2_score) ? Number(metricsToUse.r2_score).toFixed(2) : "N/A";

  // Extract training metrics if available
  const trainingMae = model_metrics?.training?.mae != null && !isNaN(model_metrics.training.mae) ? Number(model_metrics.training.mae).toFixed(1) : null;
  const trainingRmse = model_metrics?.training?.rmse != null && !isNaN(model_metrics.training.rmse) ? Number(model_metrics.training.rmse).toFixed(1) : null;
  const trainingR2Score = model_metrics?.training?.r2_score != null && !isNaN(model_metrics.training.r2_score) ? Number(model_metrics.training.r2_score).toFixed(2) : null;

  // Determine the label for the metrics (e.g., "Validation" or "Testing")
  const metricsLabel = model_metrics?.validation ? "Validation" : model_metrics?.testing ? "Testing" : "Metrics";

  // Check for missing or invalid data
  if (!modelData || !model_metrics) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-yellow-50 to-green-50">
        <div className="text-center p-4 bg-white rounded-lg shadow-sm max-w-md border border-gray-100">
          <Info className="mx-auto mb-2 text-gray-500" size={20} />
          ...
          <p className="text-xs text-gray-500">
            Model data is missing or invalid. Please check the input data and try again.
          </p>
        </div>
      </div>
    );
  ...
  // Check if no prediction data is available
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-yellow-50 to-green-50">
        <div className="text-center p-4 bg-white rounded-lg shadow-sm max-w-md border border-gray-100">
          ...
          <p className="text-xs text-gray-500">
            Please ensure prediction data (actual and predicted values) is provided and valid. Contact support if this issue persists.
          </p>
        </div>
      </div>
    );
  ...
  // Check if no metrics are available
  if (!metricsToUse) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-yellow-50 to-green-50">
        <div className="text-center p-4 bg-white rounded-lg shadow-sm max-w-md border border-gray-100">
          ...
          <p className="text-xs text-gray-500">
            Please ensure validation or testing metrics are provided. Contact support if this issue persists.
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-12 bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          {isMetricsExpanded && (
            <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                <h4 className="text-xs font-medium text-gray-700 mb-1">{metricsLabel} MAE</h4>
                <p className="text-sm font-semibold text-red-600">{mae}</p>
                <p className="text-xs text-gray-500 mt-1">
                  Average prediction error. Lower values indicate better accuracy.
                </p>
              ...
              <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                <h4 className="text-xs font-medium text-gray-700 mb-1">{metricsLabel} R² Score</h4>
                <p className="text-sm font-semibold text-green-600">{r2Score}</p>
                <p className="text-xs text-gray-500 mt-1">
                  Indicates prediction fit. Values closer to 1 signify better alignment.
                </p>
              ...
              {model_metrics.training && trainingMae && trainingRmse && trainingR2Score && (
                <>
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <h4 className="text-xs font-medium text-gray-700 mb-1">Training MAE</h4>
                    <p className="text-sm font-semibold text-red-600">{trainingMae}</p>
                    <p className="text-xs text-gray-500 mt-1">
                      Average prediction error on training data.
                    </p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <h4 className="text-xs font-medium text-gray-700 mb-1">Training RMSE</h4>
                    <p className="text-sm font-semibold text-red-600">{trainingRmse}</p>
                    <p className="text-xs text-gray-500 mt-1">
                      Measures larger errors’ impact on training data.
                    </p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                    <h4 className="text-xs font-medium text-gray-700 mb-1">Training R² Score</h4>
                    <p className="text-sm font-semibold text-green-600">{trainingR2Score}</p>
                    <p className="text-xs text-gray-500 mt-1">
                      Indicates prediction fit on training data.
                    </p>
                  </div>
                </>
              )}
