...
//                     // Apply new color convention: less than 10% = green, otherwise red.
//                     const errorColor =
//                       absPercentageError < 10 ? "text-green-600" : "text-red-600";
//                     const percentageColor =
//                       absPercentageError < 10 ? "text-green-600" : "text-red-600";
                ...
                <tbody className="text-sm">
                  {dashboardData.predictions.actual.map((actVal, i) => {
                    ...
                    const percentageError =
                      actVal !== 0 ? ((predVal - actVal) / actVal) * 100 : 0; // Avoid division by zero
                    const absPercentageError = Math.abs(percentageError);
                    // Color convention: < 10% => green, >= 10% => red
                    const errorColor =
                      absPercentageError < 10 ? "text-green-600" : "text-red-600";
                    const percentageColor =
                      absPercentageError < 10 ? "text-green-600" : "text-red-600";
                    ...
                    const analysisTime = dashboardData.predictions.analysis_time[i].replace("T", " ");

                    return (
                      <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        <td className="px-4 py-2 border">
                          {dashboardData.predictions.product_id[i]}
                        </td>
                        <td className="px-4 py-2 border">{analysisTime}</td>
                        <td className="px-4 py-2 border">{actVal}</td>
                        {/* Show predicted value with 2 decimals */}
                        <td className="px-4 py-2 border">{predVal.toFixed(2)}</td>
                        <td className={`px-4 py-2 border font-semibold ${errorColor}`}>
                          {errorVal > 0 ? `+${errorVal.toFixed(2)}` : errorVal.toFixed(2)}
                        </td>
                        <td className={`px-4 py-2 border font-semibold ${percentageColor}`}>
                          {absPercentageError.toFixed(2)}%
                        </td>
                      </tr>
                    );
                  })}
